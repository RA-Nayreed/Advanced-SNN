...
mod tests {
    ...
    #[test]
    fn distributed_graph_has_expected_shape() {
        let partition = NeuronPartition::new(10, 1, 4).unwrap();
        let graph = generate_distributed_random_graph(&partition, 3, 7).unwrap();

        assert_eq!(partition.start, 2);
        assert_eq!(partition.count, 3);
        assert_eq!(graph.global_neurons(), 10);
        assert_eq!(graph.local_start(), 2);
        assert_eq!(graph.local_neurons(), 3);
        assert_eq!(graph.fanout(), 3);
        assert_eq!(graph.row_ptr(), &[0, 3, 6, 9]);
        assert_eq!(graph.targets_global().len(), 9);
        assert_eq!(graph.weights().len(), 9);
    }

    #[test]
    fn distributed_graph_is_deterministic() {
        let partition = NeuronPartition::new(100, 2, 4).unwrap();

        let a = generate_distributed_random_graph(&partition, 8, 42).unwrap();
        let b = generate_distributed_random_graph(&partition, 8, 42).unwrap();

        assert_eq!(a, b);
    }

    #[test]
    fn different_ranks_generate_different_local_slices() {
        let p0 = NeuronPartition::new(100, 0, 4).unwrap();
        let p1 = NeuronPartition::new(100, 1, 4).unwrap();

        let g0 = generate_distributed_random_graph(&p0, 8, 42).unwrap();
        let g1 = generate_distributed_random_graph(&p1, 8, 42).unwrap();

        assert_ne!(g0.targets_global(), g1.targets_global());
        assert_eq!(g0.local_start(), 0);
        assert_eq!(g1.local_start(), 25);
    }
}
